...
$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(a,c){var b=$jscomp.checkStringArgs(this,a,"startsWith");a+="";for(var d=b.length,f=a.length,g=Math.max(0,Math.min(c|0,b.length)),h=0;h<f&&g<d;)if(b[g++]!=a[h++])return!1;return h>=f}},"es6","es3");
...
shakaDemo.preparePlayer_=function(a){shakaDemo.closeError();var b=shakaDemo.player_,c=document.getElementById("drmSettingsVideoRobustness").value,e=document.getElementById("drmSettingsAudioRobustness").value,d=["com.widevine.alpha","com.microsoft.playready","com.adobe.primetime","org.w3.clearkey"],f={abr:{},streaming:{},manifest:{dash:{}},offline:{},drm:{advanced:{}}};d.forEach(function(a){f.drm.advanced[a]={}});f.manifest.dash.clockSyncUri="https://shaka-player-demo.appspot.com/time.txt";if(!a){var g=
document.getElementById("licenseServerInput").value,h={};g&&d.forEach(function(a){h[a]=g});a={manifestUri:document.getElementById("manifestInput").value,licenseServers:h,certificateUri:document.getElementById("certificateInput").value}}f.offline.progressCallback=function(a,b){document.getElementById("progress").textContent=(100*b).toFixed(2)};b.resetConfiguration();ShakaDemoUtils.setupAssetMetadata(a,b);shakaDemo.castProxy_.setAppData({asset:a});c&&d.forEach(function(a){f.drm.advanced[a].videoRobustness=
c});e&&d.forEach(function(a){f.drm.advanced[a].audioRobustness=e});f.preferredAudioLanguage=document.getElementById("preferredAudioLanguage").value;f.preferredTextLanguage=document.getElementById("preferredTextLanguage").value;d=Number(document.getElementById("preferredAudioChannelCount").value);isNaN(d)||(f.preferredAudioChannelCount=d);d=document.getElementById("availabilityWindowOverride").value;var k=Number(d);!isNaN(k)&&d.length&&(f.manifest.availabilityWindowOverride=k);f.abr.enabled=document.getElementById("enableAdaptation").checked;
d=document.getElementById("smallGapLimit").value;!isNaN(Number(d))&&0<d.length&&(f.streaming.smallGapLimit=Number(d));f.streaming.jumpLargeGaps=document.getElementById("jumpLargeGaps").checked;f.streaming.alwaysStreamText=document.getElementById("showNative").checked;b.configure(f);window.debugConfig&&b.configure(window.debugConfig);return a};shakaDemo.computeDisabledAssets=function(){shakaDemo.onlineOptGroups_.forEach(function(a){a.disabled=!navigator.onLine})};
...
shakaDemo.storeDeleteAsset_=function(){shakaDemo.closeError();shakaDemo.offlineOperationInProgress_=!0;shakaDemo.updateButtons_(!1);var a=document.getElementById("assetList"),b=a.options[a.selectedIndex],c=new shaka.offline.Storage(shakaDemo.localPlayer_);document.getElementById("progress").textContent=(0).toFixed(2);if(b.storedContent){var e=b.storedContent.offlineUri;for(var d=b.storedContent.originalManifestUri,f=0;f<shakaAssets.testAssets.length;f++){var g=shakaAssets.testAssets[f];if(d==g.manifestUri){shakaDemo.preparePlayer_(g);
break}}e=c.remove(e).then(function(){for(var b=0;b<a.options.length;b++){var c=a.options[b];c.asset&&c.asset.manifestUri==d&&(c.isStored=!1)}return shakaDemo.refreshAssetList_()})}else{e=Promise.resolve();var h=shakaDemo.preparePlayer_(b.asset);h.certificateUri&&(e=shakaDemo.requestCertificate_(h.certificateUri).then(shakaDemo.configureCertificate_));e=e.then(function(){var d=document.getElementById("offlineName").value,e=h.name?"[OFFLINE] "+h.name:null;return c.store(h.manifestUri,{name:e||d||h.manifestUri}).then(function(){b.asset&&
(b.isStored=!0);return shakaDemo.refreshAssetList_().then(function(){for(var b=shakaDemo.offlineOptGroup_,c=0;c<b.childNodes.length;c++){var d=b.childNodes[c];d.textContent==e&&(a.selectedIndex=d.index)}})})})}e["catch"](function(a){shakaDemo.onError_(a)}).then(function(){shakaDemo.offlineOperationInProgress_=!1;shakaDemo.updateButtons_(!0);return c.destroy()})};shakaDemo.refreshAssetList_=function(){for(var a=shakaDemo.offlineOptGroup_;a.firstChild;)a.removeChild(a.firstChild);return shakaDemo.setupOfflineAssets_()};
